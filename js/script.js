...
function cargarPregunta() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    let opcionTexto = opcionKey ? (translations[currentLang]?.[opcionKey] || q.opciones[i]) : q.opciones[i];
...
const COSMOS_KNOWLEDGE = {
  ...
  respuestas: {
    es: {
      ...
      'bye': '¡Chao! 👋 Siempre puedes volver para más informações astronômicas.'
    },
    en: {
...
// Historial de conversación para aprendizaje
let conversacionHistorial = JSON.parse(localStorage.getItem('cosmos_chat_historial') || '[]');
...
function guardarConversacion(mensaje, respuesta) {
  ...
  localStorage.setItem('cosmos_chat_historial', JSON.stringify(conversacionHistorial));
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Mensajes de bienvenida bilingual
...
Ask me anything!`
  ...
  const mensajeBienvenida = mensajesBienvenida[currentLang] || mensajesBienvenida.es;
  ...
  function sendMessage() {
    ...
    // Procesar mensaje con pequeño delay para simular "pensando"
    setTimeout(() => {
